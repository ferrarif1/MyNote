...
contract XunWenGe is ERC1155, AccessControl, ERC1155Burnable, ERC2771Context {
    ...
    string private _imageUri = "https://whex.oss-cn-hangzhou.aliyuncs.com/metadata/";
    
    mapping(address => mapping(uint => bool)) balanceMap;
    mapping(address => uint[]) tokensMap;
    ...
    event BurnLog(address, uint256[], uint256[], TxEnum);
    ...
    function checkparam(
        ...
        if (enumType != TxEnum.BatchMatch) return;
    ...
    function exchangeMint(
        ...
        // 进行参数校验
        checkparam(tos, ids, amounts, txType);
    ...
    function exchangeBurn(
        ...
        accounts[0] = from;
        checkparam(accounts, ids, amounts, txType);
        emit BurnLog(from, ids, amounts, txType);
        // 判断是单个销毁还是批量
        // 单个销毁
        if (txType == TxEnum.None) {
    ...
    function getAccountBalance(address from) public view returns(BalanceInfo[] memory) {
        uint[] memory tokens = tokensMap[from];
        BalanceInfo[] memory balanceInfo = new BalanceInfo[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            uint amount = balanceOf(from, tokens[i]);
